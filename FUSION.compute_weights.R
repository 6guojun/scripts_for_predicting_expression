...
option_list = list(
  ...
  make_option("--PATH_plink", action="store", default="plink2", type='character',
...
weights.lasso = function( input , hsq , snp , out=NA ) {
	...
	arg = paste( opt$PATH_plink , " --allow-no-sex --threads 8 --bfile " , input , " --lasso " , hsq , " --out " , out , sep='' )
	system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT )
...
arg = paste( opt$PATH_plink ," --allow-no-sex --threads 8 --bfile ",opt$bfile," --pheno ",pheno.file," --keep ",pheno.file," --make-bed --out ",geno.file,sep='')
system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
if ( is.na(opt$hsq_set) ) {
    ...
    # 1. generate GRM
    arg = paste( opt$PATH_plink," --allow-no-sex --threads 8 --bfile ",opt$tmp," --make-grm-bin --out ",opt$tmp,sep='')
    system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
    ...
    if ( !file.exists( paste(opt$tmp,".hsq",sep='') ) ) {
    	cat(paste(opt$tmp,".hsq",sep=''),"does not exist, likely GCTA could not converge, skipping gene\n",file=stderr())
    	cleanup()
    	q()
    }
    
    print('printing hsq data')
    ...
    if ( opt$save_hsq ) cat( opt$out , hsq , hsq.pv , '\n' , file=paste0(opt$out,".hsq"))
    print(paste0('.hsq saved to ', opt$out,".hsq"))
...
if ( opt$crossval <= 1 ) {
    ...
    for ( i in 1:opt$crossval ) {
        ...
        arg = paste0( opt$PATH_plink ," --allow-no-sex --threads 8 --bfile ",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed")
        system(arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
    ...
    print('compute rsq+pval for each model')
    for ( mod in 1:M ) {
...
(paste( opt$out , ".wgt.RDat" , sep='' ))
print(paste0('output saved to ',opt$out , ".wgt.RDat"))
